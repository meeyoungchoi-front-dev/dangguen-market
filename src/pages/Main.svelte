<script>
    let hour = new Date().getHours();
    let min = new Date().getMinutes();
</script>
<header>
    <div class="info-bar">
    <div class="info-bar__time">{hour}:{String(min).padStart(2,"0")}</div>
        <div class="info-bar__icons">
            <img src="./assets/chart-bar.svg" alt="chart-bar">
            <img src="./assets/wifi.svg" alt="wifi">
            <img src="./assets/battery.svg" alt="battery">
        </div>
    </div>
    <div class="menu-bar">
        <div class="menu-bar__location">
            <div>역삼1동</div> 
            <div class="menu-bar__location-icon">
                <img src="./assets/bottom-arrow.svg" alt="bottom-arrow">
            </div> 
        </div>
        <div class="menu-bar__icons">
            <img src="./assets/search.svg" alt="search">
            <img src="./assets/hamburger-menu.svg" alt="hanburger-menu">   
            <img src="./assets/bell.svg" alt="bell"> 
        </div>
    </div>
</header>
<main>
    <div class="item-list">
        <div class="item-list__img">
            <img src="./assets/photo.svg" alt="image">
        </div>
        <div class="item-list__info">
            <div class="item-list__info-title">로렉스 신형 서브마리너 그린 스타벅스 판매</div>
            <div class="item-list__info-meta">
                <div class="location">도곡동</div>
                <div class="upload-time">30초전</div>
            </div>
            <div class="item-list__info-price">1,950만원</div>
        </div>
        <div class="main-bottom">
            <div class="reply">
                <img src="./assets/chat.svg" alt="reply-image">
                <span>100</span>
            </div>
            <div class="like">
                <img src="./assets/like.svg" alt="like-image">
                <span>999</span>
            </div>
        </div>
    </div>
    <div class="item-list">
        <div class="item-list__img">
            <img src="./assets/photo.svg" alt="image">
        </div>
        <div class="item-list__info">
            <div class="item-list__info-title">게이밍 pc 팝니다</div>
            <div class="item-list__info-meta">
                <div class="location">도곡동</div>
                <div class="upload-time">30초전</div>
            </div>
            <div class="item-list__info-price">100,0000원</div>
        </div>
        <div class="main-bottom">
            <div class="reply">
                <img src="./assets/chat.svg" alt="reply-image">
                <span>2</span>
            </div>
            <div class="like">
                <img src="./assets/like.svg" alt="like-image">
                <span>10</span>
            </div>
        </div>
    </div>
    <div class="item-list">
        <div class="item-list__img">
            <img src="./assets/photo.svg" alt="image">
        </div>
        <div class="item-list__info">
            <div class="item-list__info-title">게이밍 pc 팝니다</div>
            <div class="item-list__info-meta">
                <div class="location">도곡동</div>
                <div class="upload-time">30초전</div>
            </div>
            <div class="item-list__info-price">100,0000원</div>
        </div>
        <div class="main-bottom">
            <div class="reply">
                <img src="./assets/chat.svg" alt="reply-image">
                <span>2</span>
            </div>
            <div class="like">
                <img src="./assets/like.svg" alt="like-image">
                <span>10</span>
            </div>
        </div>
    </div>
    <div class="item-list">
        <div class="item-list__img">
            <img src="./assets/photo.svg" alt="image">
        </div>
        <div class="item-list__info">
            <div class="item-list__info-title">게이밍 pc 팝니다</div>
            <div class="item-list__info-meta">
                <div class="location">도곡동</div>
                <div class="upload-time">30초전</div>
            </div>
            <div class="item-list__info-price">100,0000원</div>
        </div>
        <div class="main-bottom">
            <div class="reply">
                <img src="./assets/chat.svg" alt="reply-image">
                <span>2</span>
            </div>
            <div class="like">
                <img src="./assets/like.svg" alt="like-image">
                <span>10</span>
            </div>
        </div>
    </div>
    <div class="item-list">
        <div class="item-list__img">
            <img src="./assets/photo.svg" alt="image">
        </div>
        <div class="item-list__info">
            <div class="item-list__info-title">게이밍 pc 팝니다</div>
            <div class="item-list__info-meta">
                <div class="location">도곡동</div>
                <div class="upload-time">30초전</div>
            </div>
            <div class="item-list__info-price">100,0000원</div>
        </div>
        <div class="main-bottom">
            <div class="reply">
                <img src="./assets/chat.svg" alt="reply-image">
                <span>50</span>
            </div>
            <div class="like">
                <img src="./assets/like.svg" alt="like-image">
                <span>100</span>
            </div>
        </div>
    </div>
    <div class="item-list">
        <div class="item-list__img">
            <img src="./assets/photo.svg" alt="image">
        </div>
        <div class="item-list__info">
            <div class="item-list__info-title">게이밍 pc 팝니다</div>
            <div class="item-list__info-meta">
                <div class="location">도곡동</div>
                <div class="upload-time">30초전</div>
            </div>
            <div class="item-list__info-price">100,0000원</div>
        </div>
        <div class="main-bottom">
            <div class="reply">
                <img src="./assets/chat.svg" alt="reply-image">
                <span>2</span>
            </div>
            <div class="like">
                <img src="./assets/like.svg" alt="like-image">
                <span>10</span>
            </div>
        </div>
    </div>
    <div class="item-list">
        <div class="item-list__img">
            <img src="./assets/photo.svg" alt="image">
        </div>
        <div class="item-list__info">
            <div class="item-list__info-title">게이밍 pc 팝니다</div>
            <div class="item-list__info-meta">
                <div class="location">도곡동</div>
                <div class="upload-time">30초전</div>
            </div>
            <div class="item-list__info-price">100,0000원</div>
        </div>
        <div class="main-bottom">
            <div class="reply">
                <img src="./assets/chat.svg" alt="reply-image">
                <span>2</span>
            </div>
            <div class="like">
                <img src="./assets/like.svg" alt="like-image">
                <span>10</span>
            </div>
        </div>
    </div>
    <div class="item-list">
        <div class="item-list__img">
            <img src="./assets/photo.svg" alt="image">
        </div>
        <div class="item-list__info">
            <div class="item-list__info-title">게이밍 pc 팝니다</div>
            <div class="item-list__info-meta">
                <div class="location">도곡동</div>
                <div class="upload-time">30초전</div>
            </div>
            <div class="item-list__info-price">100,0000원</div>
        </div>
        <div class="main-bottom">
            <div class="reply">
                <img src="./assets/chat.svg" alt="reply-image">
                <span>2</span>
            </div>
            <div class="like">
                <img src="./assets/like.svg" alt="like-image">
                <span>10</span>
            </div>
        </div>
    </div>
    <div class="write-btn">+글쓰기</div>
</main>
<footer>
    <div class="footer-block">
        <div class="footer-icons">
            <img src="./assets/home.svg" alt="home">
            <div class="footer-icons__desc">홈</div>                
        </div>
        <div class="footer-icons">
            <img src="./assets/paper.svg" alt="papger">
            <div class="footer-icons__desc">동네생활</div>                
        </div>
        <div class="footer-icons">
            <img src="./assets/location.svg" alt="location">
            <div class="footer-icons__desc">내 근처</div>                
        </div>
        <div class="footer-icons">
            <img src="./assets/chat.svg" alt="chat">
            <div class="footer-icons__desc">채팅</div>
        </div>
        <div class="footer-icons">
            <img src="./assets/user.svg" alt="user">
            <div class="footer-icons__desc">나의 당근</div>                
        </div>
    </div>
</footer>

<style>
    .info-bar__time {
        color: blue;
    }
</style>
